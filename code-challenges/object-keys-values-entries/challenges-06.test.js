...
const templatingWithMustache = () => {
  ...
  let characterStringArray = [];

  characters.forEach(objectInArray => {
    let template = $('#template').html();
    let html = Mustache.render(template, objectInArray);
    characterStringArray.push(html);
  })
  return characterStringArray;
};
...
// I worked with Meghan and Blake on the below, and Meghan shared what she'd worked on earlier with Jen and Matt
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(itemInArray => {
    if (Object.values(itemInArray.children).length > 0){
      array.push(itemInArray.name);
    }
  })
