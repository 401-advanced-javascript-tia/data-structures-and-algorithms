...
const createServer = () => {
  ...
  app.get('/favoritefoods', handleFoods);

  function handleGreeting(request, response){
    let greeting = 'Hello! Thank you for visiting.';
    response.status(200).send(greeting);
  }

  function handleAboutMe(request, response){
    let aboutMe = 'My name is Tia and I am a software developer. I like gardening, puzzles, and problem solving.';
    response.status(200).send(aboutMe);
  }

  function handleFoods(request, response){
    let favFoods = ['lasagna', 'garlic', 'pickled veggies'];
    response.status(200).send(`My favorite foods are: ${favFoods}`)
  }

  app.get('*', (request, response) => {
    response.status(404).response('We\'re sorry, we didn\'t recognize your request.')
  });
...
let findMaleAndFemale = (data) => {
  ...
  let genderArr = data.filter(character => character.gender === 'female' || character.gender === 'male')

  let nameArr = genderArr.map(character => {
